...
When the stored message shape changed, migrating the original `Message` table in place was not practical, so the template added `Message_v2` beside it, backfilled, and later dropped the original. The old table is gone; the version number is still in the name of the one you have. That is the problem `sdk_version` exists to avoid here - see [Migrating](/en/docs/migrating).
