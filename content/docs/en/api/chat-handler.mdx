...
| --- | --- | --- |
| `chatHandler` `POST` | Once, before anything else, for an **existing** thread | `403 Forbidden` |
| `resumeHandler` `GET` / `DELETE` | Separately on each | `204 No Content` |
| Any store call you make yourself | **Never** | - |

One check covers the whole `POST`, because a turn is a single logical operation on a single thread: load, append, stream and persist all sit behind it. The resume routes answer `204` rather than `403` on purpose, so a prober cannot tell "not your thread" from "nothing to resume".
