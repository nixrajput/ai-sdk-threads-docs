...
It returns what it removed, oldest first, so you can log or archive before the rows are gone for good. It is idempotent - a second call on an unbranched thread returns `[]` - and a thread with no active leaf prunes nothing, because "delete everything unreachable" must not collapse into "delete everything".
