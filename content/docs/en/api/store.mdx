...
Pinned rather than asserted: [`test/queries.test.ts`](https://github.com/nixrajput/ai-sdk-threads/blob/main/test/queries.test.ts) pages past the first result set by cursor and requires the deeper page to cost the same single statement, so an `OFFSET` scan creeping in fails CI.
